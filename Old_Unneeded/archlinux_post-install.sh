...
readonly ETHERNET="y"
INTERFACE=$(ls -d /sys/class/net/en*)
...
pre-install_setup() {
    ...
    pacman -Syu
    pacman -S --asexplicit pipewire lib32-pipewire wireplumber pipewire-alsa pipewire-pulse galculator kitty neofetch flameshot dunst vulkan-icd-loader lib32-vulkan-icd-loader thunar gvfs thunar-archive-plugin file-roller tumbler
...
xorg_install() {
    ...
    pacman -S --asexplicit xorg xorg-xinit bspwm sxhkd python-pywal feh rofi polybar
...
wayland_install() {
    ...
    pacman -S --asexplicit sway glfw-wayland xorg-xwayland waybar
    (
        cd /home/"$USER_NAME" || exit
...
if [ "$VM" = "y" ]; then
    ...
    pacman -S --asexplicit nvidia nvidia-utils lib32-nvidia-utils
    sed -i "s/MODULES=()/MODULES=(nvidia nvidia_modeset nvidia_uvm nvidia_drm)/" /etc/mkinitcpio.conf
    printf "[Trigger]\nOperation=Install\nOperation=Upgrade\nOperation=Remove\nType=Package\nTarget=nvidia\nTarget=linux\n# Change the linux part above and in the Exec line if a different kernel is used\n\n[Action]\nDescription=Update Nvidia module in initcpio\nDepends=mkinitcpio\nWhen=PostTransaction\nNeedsTargets\nExec=/bin/sh -c 'while read -r trg; do case \$trg in linux) exit 0; esac; done; /usr/bin/mkinitcpio -P'\n" > /etc/pacman.d/hooks/nvidia.hook

    xorg_install
elif [ "$GPU" = "amd" ]; then
    pacman -S --asexplicit mesa lib32-mesa vulkan-radeon lib32-vulkan-radeon libva-mesa-driver lib32-libva-mesa-driver mesa-vdpau lib32-mesa-vdpau
